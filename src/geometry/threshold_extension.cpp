...
std::optional<Point> algorithm::nearest_intersection_in_direction_limited(const Point& origin,
...
std::vector<Segment_w_info> algorithm::ray_shoot_intersection_limited(const std::vector<Segment_w_info>& segments) {
    ...
    for (const Segment& segment : just_segments) {
        ...
        double max_dist_squared = 16 * CGAL::to_double(segment.squared_length());

        if (segments[count].shoot_source) {
            auto hit_forward = nearest_intersection_in_direction_limited(p1, forward, tree, &segment);
            if (hit_forward) {
                double actual_dist_squared = CGAL::to_double(CGAL::squared_distance(p1, *hit_forward));
                if (actual_dist_squared <= max_dist_squared) {
        ...
        if (segments[count].shoot_target) {
            auto hit_backward = nearest_intersection_in_direction_limited(p2, backward, tree, &segment);
            if (hit_backward) {
                double actual_dist_squared =  CGAL::to_double(CGAL::squared_distance(p2, *hit_backward));
                if (actual_dist_squared <= max_dist_squared) {
